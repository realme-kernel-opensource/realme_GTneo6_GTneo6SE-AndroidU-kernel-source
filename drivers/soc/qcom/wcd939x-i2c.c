...
//#ifdef OPLUS_ARCH_EXTENDS
/* increase retry time to 1s*/
#define PM_RUNTIME_RESUME_CNT 200
//#endif /* OPLUS_ARCH_EXTENDS */
...
//#if IS_ENABLED(CONFIG_OPLUS_FEATURE_MM_FEEDBACK)
		mm_fb_audio_kevent_named(OPLUS_AUDIO_EVENTID_HEADSET_DET, MM_FB_KEY_RATELIMIT_5MIN, \
			"priv->runtime_env_counter %d underrun", priv->runtime_env_counter);
//#endif /*CONFIG_OPLUS_FEATURE_MM_FEEDBACK*/
...
static int typec_switch_get_status(struct wcd_usbss_ctxt *priv)
{
	...
	if (priv->cable_status & BIT(WCD_USBSS_USB)) {
...
//#endif /* OPLUS_ARCH_EXTENDS */
	...
	if (connect_status == WCD_USBSS_CABLE_DISCONNECT) {
		...
		case WCD_USBSS_CHARGER:
			wcd_usbss_ctxt_->cable_status &= ~BIT(WCD_USBSS_CHARGER);
			charger_switch = true;
			break;
#endif
		default:
...
#ifdef OPLUS_FEATURE_CHG_BASIC
		if (!wcd_usbss_ctxt_->cable_status && (audio_switch || charger_switch)) {
#else
